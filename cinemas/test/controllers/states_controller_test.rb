...
class StatesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create state" do
    assert_difference('State.count') do
      post states_url, params: { state: { country_id: @state.country_id, name: @state.name, status: @state.status } }
    end
  ...
  test "should update state" do
    patch state_url(@state), params: { state: { country_id: @state.country_id, name: @state.name, status: @state.status } }
